...
export default async function SchedulePage({ params }: Props) {
  ...
  const t = await getTranslations("attendeeApp.schedule");
  const tHome = await getTranslations("attendeeApp.home");
      ...
      {items.length === 0 && (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="event-body event-card-muted-text">{t("noItems")}</p>
          </CardContent>
        </Card>
      )}
              ...
              >
                <CardContent className="p-6">
                  ...
                  <div className="event-body mt-0.5 text-sm event-card-muted-text">
                    {formatTime(item.startTime, locale, eventTimezone)}
