...
export default async function AttendeeHomePage({ params }: Props) {
  ...
  const t = await getTranslations("attendeeApp.home");
  const tTypes = await getTranslations("event.types");
  ...
  const eventTypeLabel =
    event?.type && ["wedding", "birthday", "corporate", "other"].includes(event.type)
      ? tTypes(event.type as "wedding" | "birthday" | "corporate" | "other")
      : tTypes("event");
      ...
      <div className="mb-6 px-2 text-white">
        ...
        {event?.date && (
          <p className="mt-1.5 flex items-center gap-1.5 text-sm text-white/80">
            ...
            {formatDate(event.date, locale, event.timezone)}
          </p>
        )}
        {event?.location && (
        ...
        <p className="event-body mt-4 text-lg font-medium text-white">
          {t("welcome", { name: attendee?.name ?? "Guest" })}
        </p>
      </div>
          ...
          <div className="glass-card rounded-2xl p-4">
              ...
              >
                ...
                <div>
                  ...
                  <div className="event-body text-xs event-card-muted-text">
                    {formatTime(currentItem.startTime, locale, event?.timezone)}
                    {currentItem.endTime && ` – ${formatTime(currentItem.endTime, locale, event?.timezone)}`}
                  </div>
                </div>
              </div>
            )}
            {nextItem && (
              ...
              >
                ...
                />
                <div>
                  <div className="text-xs font-medium event-card-muted-text">{t("upNext")}</div>
                  <div className="event-heading font-medium event-card-title">{nextItem.title}</div>
                  <div className="event-body text-xs event-card-muted-text">
                    {formatTime(nextItem.startTime, locale, event?.timezone)}
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="glass-card rounded-2xl p-4">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                {mySeat?.label && (
                  <div className="event-body text-sm event-card-desc">{t("seat", { label: mySeat.label })}</div>
                )}
              </div>
            </div>
