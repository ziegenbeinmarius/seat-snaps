...
export function AttendeesPanel({ eventId, hasSeating = true, defaultStatus }: Props) {
  const t = useTranslations("event.attendees");
  ...
  const tableMap = new Map(tables.map((tbl) => [tbl.id, tbl.name]));
  ...
  const tabs: { label: string; value: StatusFilter; count: number }[] = [
    { label: tCommon("all"), value: "all", count: attendees.length },
    { label: tCommon("pending"), value: "pending", count: pendingCount },
    { label: tCommon("confirmed"), value: "confirmed", count: confirmedCount },
    { label: tCommon("declined"), value: "declined", count: declinedCount },
  ];
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold">
          {t("addTitle").replace("Add Attendee", "Attendees")} ({attendees.length})
          {pendingCount > 0 && (
            <Badge className="ml-2 bg-amber-100 text-amber-800 border-amber-200">
              {pendingCount} {tCommon("pending").toLowerCase()}
            </Badge>
          )}
        </h2>
      ...
      {(statusFilter === "pending" || statusFilter === "all") && pendingCount > 0 && (
        <div className="flex items-center gap-2 rounded-md bg-amber-50 border border-amber-200 px-3 py-2">
          <span className="text-sm text-amber-800 font-medium">
            {pendingCount} {tCommon("pending").toLowerCase()} RSVP{pendingCount !== 1 ? "s" : ""}
          </span>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{tCommon("none").replace("None", "Group")}</TableHead>
                <TableHead>Profile</TableHead>
                <TableHead>{t("columns.status")}</TableHead>
                {hasSeating && statusFilter !== "pending" && <TableHead>{t("columns.table")}</TableHead>}
            ...
            <TableBody>
              {filteredAttendees.map((a) => (
                <TableRow key={a.id} className={a.status === "pending" ? "bg-amber-50/50" : ""}>
                  ...
                  <TableCell>
                    {a.status === "pending" && (
                      <Badge className="bg-amber-100 text-amber-800 border-amber-200">{tCommon("pending")}</Badge>
                    )}
                    {a.status === "confirmed" && (
                      <Badge className="bg-green-100 text-green-800 border-green-200">{tCommon("confirmed")}</Badge>
                    )}
                    {a.status === "declined" && (
                      <Badge variant="outline" className="text-muted-foreground">{tCommon("declined")}</Badge>
                    )}
                  </TableCell>
      ...
      >
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingAttendee ? t("editTitle") : t("addTitle")}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-2">
            ...
            <div className="space-y-1">
              <Label>{t("fields.groupLabel")}</Label>
              <Input
                placeholder={t("placeholders.groupLabel")}
            ...
            <div className="space-y-1">
              <Label>{t("fields.conversationStarters")}</Label>
              <Input
                placeholder={t("placeholders.conversationStarters")}
