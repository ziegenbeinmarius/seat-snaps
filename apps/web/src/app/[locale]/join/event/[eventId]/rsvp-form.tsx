...
export function RsvpForm({ eventId, eventTitle, onBack }: Props) {
  ...
  function validate() {
    ...
    if (description.length > 200) e.description = t("validation.descriptionTooLong");
    return e;
  }
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (message?.toLowerCase().includes("email")) {
        ...
        setErrors({ submit: message ?? t("validation.registrationFailed") });
      }
    }
  }
  ...
  return (
    ...
    >
      ...
      <form
        ...
        noValidate
        className="flex-1 overflow-y-auto p-6 space-y-4 pb-36"
      >
        {/* Name */}
        <div>
          <label
          ...
          >
            {t("fields.name")} <span style={{ color: "hsl(0 70% 50%)" }}>*</span>
          </label>
          <input
            type="text"
            autoComplete="name"
          ...
          />
          {errors.name && (
            <p className="mt-1 text-xs" style={{ color: "hsl(0 70% 50%)" }}>
              {errors.name}
            </p>
          )}
        </div>
        ...
        <div>
          <label
            ...
            style={{ color: "hsl(28 8% 42%)" }}
          >
            {t("fields.email")} <span style={{ color: "hsl(0 70% 50%)" }}>*</span>
          </label>
          <input
            type="email"
            autoComplete="email"
        ...
        <div>
          ...
          >
            {t("fields.aboutYou")}{" "}
            <span className="normal-case font-normal" style={{ color: "hsl(28 8% 60%)" }}>
              {tCommon("optional")}
            </span>
          </label>
          <textarea
            ...
            placeholder={t("placeholders.aboutYou")}
            rows={3}
            maxLength={200}
        ...
        <div>
          <label
            ...
            style={{ color: "hsl(28 8% 42%)" }}
          >
            {t("fields.conversationStarters")}{" "}
            <span className="normal-case font-normal" style={{ color: "hsl(28 8% 60%)" }}>
              {tCommon("optional")}
            </span>
          </label>
          <p className="mb-2 text-xs" style={{ color: "hsl(28 8% 58%)" }}>
            {t("conversationStartersHint")}
          </p>
            ...
            {starters.map((starter, index) => (
              <div key={index} className="flex items-center gap-2">
                <input
                  ...
                  placeholder={t("placeholders.starter")}
      ...
      >
        ...
        >
          {rsvpMutation.isPending ? t("registering") : t("registerButton")}
        </button>
      </div>
    </div>
  );
